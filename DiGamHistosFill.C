#define DiGamHistosFill_cxx
#include "DiGamHistosFill.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
#include <iostream>
using namespace std;
//using namespace DiGamHistosFill;
...
void DiGamHistosFill::Loop()
{
//   In a ROOT session, you can do:
//      root> .L DiGamHistosFill.C
//      root> DiGamHistosFill t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  */
  ...
  if (ds=="2023B" || ds=="2023Cv123" || ds=="2023Cv4" || ds=="2023D")
    sera = "2023";
  if (ds=="2024B" || ds=="2024C" || ds=="2024D" || ds=="2024E" ||
      ds=="2024F" || ds=="2024G")
    sera = "2024";
  assert(sera!="");
  ...
  if (TString(ds.c_str()).Contains("2024"))
    ...
    LoadJSON("files/Cert_Collisions2024_378981_385863_Golden.json");
  ...
  TFile *fout = new TFile(Form("rootfiles/DiGamHistosFill_%s_%s_%s.root",
			       isMC ? "mc" : "data",
  ...
  */
  ...
  digamHistos *hgg(0);
  if (doDiGam) {
   if (debug) cout << "Setup doDiGam " << endl << flush;
      
    fout->mkdir("DiGam");
    fout->cd("DiGam");

    digamHistos *h = new digamHistos();
    hgg = h;

    // 1 GeV binning
    h->hmgg = new TH1D("hmgg",";m_{#gamma#gamma} (GeV)",13600,0,13600);
    h->hmgg_13 = new TH1D("hmgg_13",";m_{#gamma#gamma} (GeV)",13600,0,13600);
    h->hmgg_200 = new TH1D("hmgg_200",";m_{#gamma#gamma} (GeV)",13600,0,13600);

    // 1% binning
    vector<double> vx;
    vx.push_back(50.);
    for (int i = 0; vx[i]*1.01<10000.; ++i) {
      vx.push_back(vx[i]*1.01);
    }
  ...
  /*
  map<int, map<int, map<int, BasicHistos2*> > > mBasicHistos;
  for (unsigned int ialpha = 0; ialpha != alphas.size(); ++ialpha) {
    ...
    for (unsigned int ips = 0; ips != nps; ++ips) {
      ...
      BasicHistos2 *pmh = new BasicHistos2();
      BasicHistos2& mh = (*pmh);
      mh.hn = hn;
      mh.hxsec = hxsec;
      mh.prpt = prpt;
  ...
  */
  ...
  TLorentzVector gam2, gengam2, gamorig2, rawgam2;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if (nPhoton > nPhotonMax) {
      cout << "Error: nPhoton="<<nPhoton
    ...
    int iGamGen2(-1), iGam2(-1);
    int iGamOrig2(-1); // for QCD bkg
    gengam2.SetPtEtaPhiM(0,0,0,0);
    gam2.SetPtEtaPhiM(0,0,0,0);
    rawgam2.SetPtEtaPhiM(0,0,0,0);
	...
	iGamGen = i;
      ...
      if (gengam2.Pt()>0 && gengam2.DeltaR(gami)<0.2 && iGamGen2==-1) {
	iGamGen2 = i;
      } 
    ...
    rawgam2 = gam2;
    if (iGam2!=-1 && Photon_seedGain[iGam2]==1 && !isMC) {
      //gam *= 1./1.01;
      // minitools/drawGainVsPt.C (add R_6/12+R_1/6, take MPF+statTowardsDB)
      if (!isRun3) gam2 *= 1./1.011; // MPF=1.13+/-0.04%, DB=1.05+/-0.08%
      if ( isRun3) gam2 *= 1./1.017; // MPF=1.74+/-0.07%, DB=1.41+/-0.16%
    }
    if (iGam!=-1 && !isRun3) {
    ...
    */
    ...
    // both of the leading jets with genjet taking the place of the photon
    int iFox(-1), iFox2(-1);
    //fox.SetPtEtaPhiM(0,0,0,0);
    //fox2.SetPtEtaPhiM(0,0,0,0);
    ...
    if (isQCD && nJet>=2) {
      ...
      iFox2 = ((jentry+1)%2); // "random" selection from two leading jets
      // Jet_genJetIdx would be great, but only there for UL18 nAOD? Maybe there
      int k = iFox;//Jet_genJetIdx[iFox];
      int k2 = iFox2;//Jet_genJetIdx[iFox];
      if (k>=0 && k<nGenJet && k2>=0 && k2<nGenJet) {
	gam.SetPtEtaPhiM(GenJet_pt[k], GenJet_eta[k], GenJet_phi[k],
			 GenJet_mass[k]);
	gam2.SetPtEtaPhiM(GenJet_pt[k2], GenJet_eta[k2], GenJet_phi[k2],
			 GenJet_mass[k2]);
	...
	double area = 0.5;//Jet_area[iFox];
	gam *= (gam.Pt()>0 ? 1 - 3.5*area/gam.Pt() : 1.);
	rawgam = gam;
	gam2 *= (gam2.Pt()>0 ? 1 - 3.5*area/gam2.Pt() : 1.);
	rawgam2 = gam2;
    ...
    */
	...
	//h2gametaphi4->Fill(gam.Eta(), gam.Phi(), w);
      ...
      bool pass_basic = (pass_trig && pass_filt && pass_ngam);// && pass_gameta);
      ...
      //	      pass_basic, pass_bal, pass_mpf, pass_basic_ext,
      //	      pass_jeteta, pass_alpha100, pass_all, pass_gen};
      bool cut[nc] = {pass_trig, pass_ngam, pass_gameta, pass_basic};
      bool passcuts(true);
      for (int i = 0; i != nc; ++i) {
	...
	*/
	...
	if (pass_basic) {
	  ...
	  if (gam.Pt()>230.) {
	    h->hmgg_200->Fill(mgg, w);
	    h->hmgg1_200->Fill(mgg, w);
	  }
...
} // DiGamHistosFill::Loop()


// Code originally from jetphys/HistosFill.C
void DiGamHistosFill::PrintInfo(string info, bool printcout)
{
...
// Code originally from jetphys/HistosFill.C
bool DiGamHistosFill::LoadJSON(string json)
{
  ...
  while (file >> s and sscanf(s.c_str(),"\"%d\":",&rn)==1) {
    if (_gh_debug2) PrintInfo(Form("\"%d\": ",rn),true);

    while (file.get(c) and c==' ') {};
    if (_gh_debug2) { PrintInfo(Form("%c",c),true); assert(c=='['); }
    ++nrun;
    ...
    while (!endrun and file >> s >> s2 and (sscanf((s+s2).c_str(),"[%d,%d]%s",&ls1,&ls2,s1)==3 or (file >> s3 and sscanf((s+s2+s3).c_str(),"[%d,%d]%s",&ls1,&ls2,s1)==3))) {
      ...
      if (_gh_debug2) PrintInfo(Form("[%d,%d,'%s']",ls1,ls2,s1),true);
      ...
      endrun = (s2=="]," || s2=="]}");
      if (_gh_debug2 and !endrun and s2!=",") { PrintInfo(string("s1: ")+s2,true); assert(s2==","); }
    } // while ls
    if (_gh_debug2) PrintInfo("",true);

    if (s2=="]}") continue;
    else if (_gh_debug2 and s2!="],") PrintInfo(string("s2: ")+s2,true); assert(s2=="],");
  } // while run
...
void DiGamHistosFill::LoadPU() {

  cout << endl << "DiGamHistosFill::LoadPU" << endl << flush;
  TDirectory *curdir = gDirectory;
