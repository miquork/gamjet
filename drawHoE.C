...
void drawHoE() {
  ...
  TH2D *h2d = (TH2D*)fd->Get("control/h2hoevspt"); assert(h2d);
  TH2D *h2m = (TH2D*)fm->Get("control/h2hoevspt"); assert(h2m);
  ...
  h2d->GetZaxis()->SetRangeUser(1e-4,1.0);
  h2d->Draw("COLZ");
  ...
  h2m->GetZaxis()->SetRangeUser(1e-4,1.0);
  h2m->Draw("COLZ");
